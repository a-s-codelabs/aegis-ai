...
export default function DashboardPage() {
  ...
  const handleDivertToAI = () => {
    if (incomingCall) {
      // Simulate diverting to AI - start monitoring
      setActiveCall({
        number: incomingCall.number,
        risk: Math.floor(Math.random() * 30) + 50, // 50-80% risk for unknown
        keywords: [],
        transcript: [
          {
            speaker: 'AI Agent',
  ...
  return (
    <>
      {/* Incoming Call Full Screen Overlay - Outside AppLayout to hide navbars */}
      {incomingCall ? (
